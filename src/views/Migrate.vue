      ...
      <v-layout row wrap>
        <v-flex xs12>
          <v-card dark color="indigo">
            <v-card-text class="px-0"><h1>剩余时间倒计时</h1></v-card-text>
          </v-card>
        </v-flex>
        <v-flex lg1>
           <v-card color="blue lighten-3">
             <v-card-text class="px-0"><h2>{{getDay}}</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg2 >
           <v-card color="blue lighten-3" >
             <v-card-text class="px-0"><h2>Days</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg1>
           <v-card color="blue lighten-3">
             <v-card-text class="px-0"><h2>{{getHour}}</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg2>
           <v-card color="blue lighten-3">
             <v-card-text class="px-0"><h2>Hours</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg1>
           <v-card>
             <v-card-text class="px-0"><h2>{{getMin}}</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg2>
           <v-card>
             <v-card-text class="px-0"><h2>Mins</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg1>
           <v-card>
             <v-card-text class="px-0"><h2>{{getSec}}</h2></v-card-text>
           </v-card>
        </v-flex>
         <v-flex lg2>
           <v-card>
             <v-card-text class="px-0"><h2>Sec</h2></v-card-text>
           </v-card>
        </v-flex>
        <v-flex xs3>
          <v-card dark color="indigo">
  ...
  computed: {
    getDay(){
        if(this.migrateData != null && this.migrateData.remainTime !=null)
          return this.migrateData.remainTime.split(",")[0].replace("days" , "").trim()
      },
    getHour(){if(this.migrateData != null && this.migrateData.remainTime !=null) return this.migrateData.remainTime.split(",")[1].replace("hours" , "").trim()},
    getMin(){if(this.migrateData != null && this.migrateData.remainTime !=null) return this.migrateData.remainTime.split(",")[2].replace("minutes" , "").trim()},
    getSec(){if(this.migrateData != null && this.migrateData.remainTime !=null) return this.migrateData.remainTime.split(",")[3].replace("seconds" , "").trim()},
